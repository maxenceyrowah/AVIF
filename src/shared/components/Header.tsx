          ...
          <nav className="hidden md:flex space-x-8">
            {MENUS.map((item) => (
              ...
              >
                <Link
                  to={item.path}
                  className="text-white hover:text-yellow-400 transition py-2 flex items-center"
                >
                  {item.name}
                ...
                >
                  {item.submenu.map((subItem) => (
                    <Link
                      key={subItem.name}
                      to={subItem.path}
                      className="block px-4 py-2 text-white hover:bg-gray-800 hover:text-yellow-400 transition-all duration-200"
                    >
                      {subItem.name}
                    </Link>
                  ))}
              ...
              {MENUS.map((item) => (
                <div key={item.name} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p
                      // to={item.path}
                      className="text-white hover:text-yellow-400 transition py-2 flex-grow"
                    >
                      {item.name}
                    </p>
                    <button
